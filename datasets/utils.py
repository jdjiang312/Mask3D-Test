...
def voxelize(
    ...
    # Original benchmark test files contain segments only (one label column),
    # while the fallen-tree test split has full semantic/instance GT. Preserve
    # the old unlabeled-test path and use the normal instance-target path when
    # GT columns are present.
    is_unlabeled_test = mode == "test" and all(
        sample_labels.ndim == 2 and sample_labels.shape[1] == 1
        for sample_labels in input_dict.get("labels", [])
    )

    if is_unlabeled_test:
