...
class SemanticSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        elif self.dataset_name == "stpls3d_fallentree":
            self.color_map = {
                0: [128, 128, 128],  # other
                1: [165, 82, 42],  # fallentree
            }
        ...
        if self.cache_data:
            ...
            for i in range(len(self._data)):
                self._data[i]["data"] = self._load_processed_points(
                    self.data[i]["filepath"]
                )
                if self.on_crops:
    ...
    def __getitem__(self, idx: int):
        ...
        if self.cache_data:
            ...
            points = self._load_processed_points(self.data[idx]["filepath"])

        if "train" in self.mode and self.dataset_name in [
            "s3dis",
            "stpls3d",
            # "stpls3d_fallentree",
        ]:
        ...
        if self.dataset_name in ["stpls3d", "stpls3d_fallentree"]:
    ...
    @staticmethod
    def _load_processed_points(filepath):
        """Load original NumPy files and the task-specific PyTorch tensors."""
        filepath = str(filepath).replace("../../", "")
        if Path(filepath).suffix == ".pth":
            points = torch.load(filepath, map_location="cpu")
