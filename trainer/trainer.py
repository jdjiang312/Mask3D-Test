...
class RegularCheckpointing(pl.Callback):
    """Save compatible last/best checkpoints after every validation epoch."""
    ...
    def on_validation_end(
        ...
        if current is None:
            raise RuntimeError(
                f"Checkpoint metric {self.monitor!r} was not logged"
            )
        current = float(current.detach().cpu() if torch.is_tensor(current) else current)
        is_best = self.best_score is None or current > self.best_score
        if is_best:
            self.best_score = current

        save_dir = Path(pl_module.config.general.save_dir)
        save_dir.mkdir(parents=True, exist_ok=True)
        trainer.save_checkpoint(str(save_dir / self.last_filename))
        if is_best:
            trainer.save_checkpoint(str(save_dir / self.best_filename))
        print(
            f"Saved {self.last_filename}; "
            + (f"updated {self.best_filename}" if is_best else "best unchanged")
        )
...
class InstanceSegmentation(pl.LightningModule):
    ...
    def eval_instance_step(
        ...
        evaluation_dataset = self._active_evaluation_dataset()
        label_offset = evaluation_dataset.label_offset
        ...
        if evaluation_dataset.dataset_name == "scannet200":
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            all_pred_classes[
                bid
            ] = evaluation_dataset._remap_model_output(
                all_pred_classes[bid].cpu() + label_offset
            )

            has_ground_truth = (
                len(target_full_res) > bid
                and "masks" in target_full_res[bid]
                and "labels" in target_full_res[bid]
            )
            if has_ground_truth:
                target_full_res[bid][
                    "labels"
                ...
                self.gt_instances[file_names[bid]] = {
                    "masks": target_full_res[bid]["masks"]
                    .cpu()
                    .detach()
                    .numpy()
                    .astype(bool),
                    "classes": np.asarray(target_full_res[bid]["labels"]),
                }
            ...
            data_index = int(idx[bid])
            self.scan_metadata[file_names[bid]] = evaluation_dataset.data[
                data_index
            ]
            ...
            if self.config.general.export:
                if evaluation_dataset.dataset_name == "stpls3d":
    ...
    def _is_testing(self):
        ...
        return "test" in str(getattr(trainer_state, "fn", "")).lower()

    def _active_evaluation_dataset(self):
        if self._is_testing():
            return self.test_dataset
        return self.validation_dataset

    def _object_level_metrics(self):
        scene_metrics = {}
        for scene_name, prediction in self.preds.items():
            if scene_name not in self.gt_instances:
                raise RuntimeError(
                    f"No full-resolution GT instances for {scene_name}"
                )
            ground_truth = self.gt_instances[scene_name]
            scene_metrics[scene_name] = match_instances(
                prediction,
                ground_truth["masks"],
                ground_truth["classes"],
                score_threshold=self.config.general.object_score_threshold,
                iou_threshold=self.config.general.object_iou_threshold,
                valid_class_ids=(1,),
            )
        return scene_metrics, aggregate_scene_metrics(scene_metrics, (1,))

    def _metrics_report(
        self, log_prefix, ap_class_results, ap_results, scene_metrics, total
    ):
        score_threshold = float(self.config.general.object_score_threshold)
        iou_threshold = float(self.config.general.object_iou_threshold)
        lines = []
        if log_prefix == "test":
            lines.extend(
                [
                    "#" * 88,
                    f"Per-scan metrics @ IoU={iou_threshold:.2f}, score>={score_threshold:.2f}",
                    "{:<28}: {:>10} {:>11} {:>11} {:>8} {:>8} {:>8}".format(
                        "scan/block", "Recall", "Precision", "F1-Score", "TP", "FP", "FN"
                    ),
                    "#" * 88,
                ]
            )
            for scene_name, result in scene_metrics.items():
                lines.append(
                    "{:<28}: {:>10.3f} {:>11.3f} {:>11.3f} {:>8d} {:>8d} {:>8d}".format(
            ...
            lines.extend(
                [
                    "#" * 96,
                    f"Epoch {self.current_epoch + 1} validation "
                    f"({datetime.now().isoformat(timespec='seconds')})",
                    f"Object metrics @ IoU={iou_threshold:.2f}, score>={score_threshold:.2f}",
                ]
            )
        ...
        lines.extend(
            [
                "#" * 96,
                "{:<15}: {:>10} {:>10} {:>10} {:>10} {:>11} {:>11}".format(
                    "what", "AP", "AP_50%", "AP_25%", "Recall", "Precision", "F1-Score"
                ),
                "#" * 96,
            ]
        )
        ...
        evaluation_dataset = self._active_evaluation_dataset()
        class_name_by_id = {
            int(class_id): info["name"]
            for class_id, info in evaluation_dataset.label_info.items()
        }
        for class_id in (1,):
            class_name = class_name_by_id[class_id]
            ap_values = ap_class_results.get(
                class_name, {"ap": 0.0, "ap50": 0.0, "ap25": 0.0}
            )
            object_values = total["per_class"][class_id]
            lines.append(
                "{:<15}: {:>10.3f} {:>10.3f} {:>10.3f} {:>10.3f} {:>11.3f} {:>11.3f}".format(
                    class_name,
                    ap_values["ap"],
                    ap_values["ap50"],
                    ap_values["ap25"],
                    object_values["recall"],
                    object_values["precision"],
                    object_values["f1"],
                )
            )
        lines.extend(
            [
                ...
                "{:<15}: {:>10.3f} {:>10.3f} {:>10.3f} {:>10.3f} {:>11.3f} {:>11.3f}".format(
                    "average",
                    ap_results.get(f"{log_prefix}_mean_ap", 0.0),
                    ap_results.get(f"{log_prefix}_mean_ap_50", 0.0),
                    ap_results.get(f"{log_prefix}_mean_ap_25", 0.0),
                    total["recall"],
                    total["precision"],
                    total["f1"],
                ),
    ...
    def _write_evaluation_outputs(
        ...
        report = self._metrics_report(
            log_prefix, ap_class_results, ap_results, scene_metrics, total
        )
        log_path = save_dir / ("test.log" if log_prefix == "test" else "train.log")
        with open(log_path, "w" if log_prefix == "test" else "a") as log_file:
            log_file.write(report)
        print(report)

        if log_prefix == "test" and self.config.general.save_las:
            las_dir = Path(self.config.general.las_output_dir)
            for scene_name, prediction in self.preds.items():
                raw_filepath = self.scan_metadata[scene_name]["raw_filepath"]
                write_prediction_las(
                    raw_filepath,
                    prediction,
                    las_dir / f"{Path(scene_name).stem}.las",
                )
    ...
    def eval_instance_epoch_end(self):
        evaluation_dataset = self._active_evaluation_dataset()
        log_prefix = "test" if self._is_testing() else "val"
        ap_results = {}
        ap_class_results = {}
        ...
        for class_id in box_ap_50[-1].keys():
            class_name = evaluation_dataset.label_info[class_id]["name"]
        ...
        if evaluation_dataset.dataset_name in [
            "scannet",
            "stpls3d",
            "stpls3d_fallentree",
            "scannet200",
        ]:
            gt_data_path = f"{evaluation_dataset.data_dir[0]}/instance_gt/{evaluation_dataset.mode}"
        else:
            gt_data_path = f"{evaluation_dataset.data_dir[0]}/instance_gt/Area_{self.config.general.area}"
        ...
        try:
            if evaluation_dataset.dataset_name == "s3dis":
                new_preds = {}
            ...
            elif evaluation_dataset.dataset_name in [
                "stpls3d",
                "stpls3d_fallentree",
            ]:
                new_preds = {}
                ...
                evaluate(
                    ...
                    gt_data_path,
                    pred_path,
                    dataset=evaluation_dataset.dataset_name,
                )
            else:
                evaluate(
                    self.preds,
            ...
            with open(pred_path, "r") as fin:
                for line_id, line in enumerate(fin):
                    ...
                    ap_class_results[class_name] = {
                        key: 0.0 if math.isnan(value) else value
                        for key, value in ap_values.items()
                    }
        ...
        scene_metrics, total_object_metrics = self._object_level_metrics()
        ap_results[f"{log_prefix}_mean_recall"] = total_object_metrics[
            "recall"
        ]
        ap_results[f"{log_prefix}_mean_precision"] = total_object_metrics[
            "precision"
        ]
        ap_results[f"{log_prefix}_mean_f1"] = total_object_metrics["f1"]
        fallentree_metrics = total_object_metrics["per_class"][1]
        ap_results[f"{log_prefix}_fallentree_recall"] = fallentree_metrics[
            "recall"
        ]
        ap_results[f"{log_prefix}_fallentree_precision"] = fallentree_metrics[
            "precision"
        ]
        ap_results[f"{log_prefix}_fallentree_f1"] = fallentree_metrics["f1"]

        self.log_dict(ap_results)
        self._write_evaluation_outputs(
            log_prefix,
            ap_class_results,
            ap_results,
            scene_metrics,
            total_object_metrics,
        )

        if not self.config.general.export and os.path.exists(base_path):
