...
def evaluate(
    ...
    # Dataset-specific label metadata only. The AP matching, thresholds,
    # integration and averaging below are the original Mask3D evaluator.
    if dataset == "stpls3d_fallentree":
        opt["min_region_sizes"] = np.array([10])
        CLASS_LABELS = ["fallentree"]
        VALID_CLASS_IDS = np.array([1])
        ID_TO_LABEL = {1: "fallentree"}
        LABEL_TO_ID = {"fallentree": 1}
